...
import {
  ...
  hasRole,
  hasGroup,
} from "@appmana-public/auth-proxy-common";
...
const argv = yargs(hideBin(process.argv))
  ...
  .option("well-known-authorize", {
    ...
    description:
      'JSON string configuration for authorize policy (repeatable). E.g. \'{"issuer": "...", "domains": ["example.com"]}\'',
    default: [] as string[],
    coerce: (arg) => arg.map(String),
  })
  ...
  fastify.addHook("onRequest", async (request, reply) => {
    ...
    if (allAllowedDomains.length > 0) {
      try {
        ...
        const isAllowed =
          micromatch.isMatch(hostToCheck, allAllowedDomains as string[]) ||
