...
test('Policy Chain - First Applicable Strategy', async (t) => {
    ...
    // Mock Policies
    const allowPolicy: PolicyFunction = async () => ({ decision: 'ALLOW', modifiedRequest: { headers: { 'x-test': 'allow' } } });
    const denyPolicy: PolicyFunction = async () => ({ decision: 'DENY' });
    const skipPolicy: PolicyFunction = async () => ({ decision: 'SKIP' });
    const modifyPolicy: PolicyFunction = async () => ({ decision: 'SKIP', modifiedRequest: { headers: { 'x-mod': 'skip' } } }); // Modifying on SKIP? Usually ignored unless Accumulated. But our engine ignores SKIP results currently?
